...
from basicsr.archs.atm import ATMLayer
...
class Spatial_Attention(nn.Module):
    ...
    def forward(self, qkv, H, W, mask=None):
        ...
        '''
        b = q.shape[0]        
        agent_tokens = q.permute(0, 2, 1, 3).reshape(b, self.H_sp, self.W_sp, C).permute(0, 3, 1, 2)
        agent_tokens = self.pool(agent_tokens).reshape(b, C, -1).permute(0, 2, 1)
        #agent_tokens = self.pool(agent_tokens.reshape(-1, self.H_sp, self.W_sp, C).permute(0, 3, 1, 2)).reshape(-1, C, self.H_sp* self.W_sp).permute(0, 2, 1)
        agent_tokens = agent_tokens.reshape(b, 16, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, 
        ...
        '''
        self.channel_interaction_c = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(dim, dim // 8, kernel_size=1),
            nn.BatchNorm2d(dim // 8),
            nn.GELU(),
        ...
        '''
        self.spatial_interaction_c = nn.Sequential(
            nn.Conv2d(dim, dim // 16, kernel_size=1),
            nn.BatchNorm2d(dim // 16),
            nn.GELU(),
            nn.Conv2d(dim // 16, 1, kernel_size=1)
    ...
    def forward(self, x, H, W):
        ...
        assert L == H * W, "flatten img_tokens has wrong size"
        _x = x.view(B, H, W, C)
        offset = self.offset_layer(_x).repeat_interleave(self.share_dim, dim=-1).permute(0, 3, 1, 2)  # [B, H, W, 2C/S] -> [B, 2C, H, W]
        attened_xc = self.atm(_x, offset).view(B, L, C)

        qkv = self.qkv(x)
        #qkv_c = qkv.contiguous().reshape(B, L, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)#(Index, B, self.num_heads, L, C//self.num_heads)       
        ...
        channel_map = self.channel_interaction(attened_xc.permute(0, 2,1).view(B, C, H, W)).permute(0, 2, 3, 1).contiguous().view(B, 1, C)
        ...
        # Adaptive Interaction Module (AIMc)
        # C-Map (before sigmoid)
        #attention_reshape_c = attened_xc.transpose(-2,-1).contiguous().view(B, C, H, W)##############
        #channel_map_c = self.channel_interaction_c(attention_reshape_c)####################
        # S-Map (before sigmoid)

        #spatial_map_c = self.spatial_interaction_c(conv_x).permute(0, 2, 3, 1).contiguous().view(B, L, 1)####################
        spatial_map_c = self.spatial_interaction_c(attened_x.permute(0, 2,1).view(B, C, H, W)).permute(0, 2, 3, 1).contiguous().view(B, L, 1)
        # S-I
        attened_xc = attened_xc * torch.sigmoid(spatial_map_c)
        # C-I
        #conv_x = conv_x * torch.sigmoid(channel_map_c)#######################
        
        #conv_x = conv_x.permute(0, 2, 3, 1).contiguous().view(B, L, C)#########################
        
        #x = attened_x + conv_x + attened_xc
        x = attened_x + attened_xc
...
class Adaptive_Channel_Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        q = q.transpose(-2, -1)#(B, num_heads, C // self.num_heads, N)
        k = k.transpose(-2, -1)
        v = v.transpose(-2, -1)
        ...
        attn = (q @ k.transpose(-2, -1)) * self.temperature#(B, num_heads, c//self.num_heads, c//self.num_heads)
...
class DATB(nn.Module):
    def __init__(self, dim, num_heads, reso=64, split_size=[2,4],shift_size=[1,2], expansion_factor=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        #self.ffn = Mlp(in_features=dim, hidden_features=ffn_hidden_dim, out_features=dim, act_layer=act_layer)#afformer
